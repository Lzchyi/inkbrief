...
def _race_payload(
    sessions: dict[str, object], *, include_race_time: bool = True
) -> dict[str, object]:
...
def _parse(sessions: dict[str, object]):
    return parse_jolpica_snapshot(
        _race_payload(sessions),
        _driver_payload(),
...
def test_parses_normal_weekend_and_top_three_standings() -> None:
    ...
    assert snapshot.event_name == "Belgian Grand Prix"
    assert snapshot.circuit_id == "spa"
...
def test_parses_sprint_weekend_without_assuming_fp2_or_fp3() -> None:
    snapshot = _parse(
        {
            "FirstPractice": {"date": "2026-07-24", "time": "10:00:00Z"},
            "SprintQualifying": {"date": "2026-07-24", "time": "14:00:00Z"},
            "Sprint": {"date": "2026-07-25", "time": "10:00:00Z"},
            "Qualifying": {"date": "2026-07-25", "time": "14:00:00Z"},
        }
    )
...
def test_client_uses_jolpica_endpoints_without_live_network_access() -> None:
    ...
    assert snapshot.round_number == 14
